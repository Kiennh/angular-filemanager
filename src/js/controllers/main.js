(function(angular, $) {
    ...
    angular.module('FileManagerApp').controller('FileManagerCtrl', [
        ...
        function($scope, $rootScope, $window, $translate, fileManagerConfig, Item, FileNavigator, ApiMiddleware) {
            ...
            $scope.fileList = [];
            $scope.temps = [];
            $scope.modalsLoaded = function () {
                  initMarkdownEditor('mdTextArea', 'mdOut');
            };
            ...
            $scope.openRunItem = function() {
                $scope.modal('runX');
            };
            
            $scope.runItem = function() {
                var item = $scope.singleSelection();
                $scope.apiMiddleware.run(item).then(function() {
                    $scope.fileNavigator.refresh();
                    $scope.modal('runX', true);
                });
            ...
            $scope.openEditItem = function() {
                ...
                $scope.apiMiddleware.getContent(item).then(function(data) {
                    ...
                    updateTextArea(data.result);
                });
                if (item.model.name.endsWith('.md')) {
                    $scope.modal('editMD');
                } else {
                    $scope.modal('edit');
                }
